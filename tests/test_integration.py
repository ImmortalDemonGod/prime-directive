...
class TestIntegrationMockMode:
    ...
    def mock_config(self):
        ...
        mock_cfg.system.openai_api_url = "https://api.openai.com/v1/chat/completions"
    ...
    def test_freeze_in_mock_mode(self, mock_config):
        ...
        with patch("prime_directive.bin.pd.load_config", return_value=mock_config):
            with patch("prime_directive.bin.pd.init_db", new_callable=AsyncMock):
                with patch("prime_directive.bin.pd.get_session") as mock_session:
                    ...
                    with patch("prime_directive.bin.pd.dispose_engine", new_callable=AsyncMock):
                        result = runner.invoke(app, ["freeze", "test-repo", "--no-interview"])
...
class TestChaosOllamaDown:
    ...
    @pytest.mark.asyncio
    async def test_ollama_down_returns_error_message(self, config_ollama_primary):
        """When Ollama is down and no fallback, should return error message."""
        import httpx
        from prime_directive.core.scribe import generate_sitrep
        
        with patch("prime_directive.core.ai_providers.httpx.AsyncClient") as mock_client:
...
class TestGitParsingEdgeCases:
    ...
    async def test_git_status_with_renamed_files(self, tmp_path):
        ...
        subprocess.run(["git", "add", "old_name.txt"], cwd=repo, capture_output=True)
