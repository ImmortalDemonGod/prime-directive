...
def mock_importlib():
    ...
    """
    with patch(
        "prime_directive.core.auto_installer.importlib.util.find_spec"
    ) as mock:
        yield mock
...
def mock_subprocess():
    ...
    """
    with patch(
        "prime_directive.core.auto_installer.subprocess.check_call"
    ) as mock:
        yield mock
...
def test_ensure_packages_missing_no_auto_install(
    mock_importlib, mock_subprocess, mock_sys_prefix
):
...
def test_ensure_packages_missing_auto_install_success(
    mock_importlib, mock_subprocess, mock_sys_prefix
):
...
def test_ensure_packages_not_allowlisted(
    mock_importlib, mock_subprocess, mock_sys_prefix
):
