...
async def _run_git_command(
    ...
    *,
    ...
    Parameters:
        repo_path (str): Filesystem path used as the subprocess working directory (the repository root).
        args (list[str]): Command name and arguments to execute (e.g., ["git", "status", "--porcelain"]).
        timeout_seconds (float): Seconds to wait for the process to finish before killing it.
    
    Returns:
        tuple[int, str, str]: (returncode, stdout, stderr) where returncode is the process exit code (uses 1 if unavailable), and stdout/stderr are decoded strings.
    
    Raises:
        asyncio.TimeoutError: If the command does not complete within timeout_seconds; the subprocess is killed before the exception is re-raised.
    """
...
async def get_last_touched(repo_path: str) -> Optional[float]:
    ...
    Attempts to list tracked and untracked files via `git ls-files -c -o --exclude-standard` and returns the largest filesystem modification time (seconds since the epoch) among those files.
    
    Parameters:
        repo_path (str): Filesystem path to the repository root.
    
    Returns:
        float | None: The most recent modification time in seconds since the epoch, or `None` if the repository is not available, no files are found, the git command fails, or an error occurs.
    """
...
async def get_status(repo_path: str) -> GitStatus:
    ...
    Parameters:
        repo_path (str): Filesystem path to the repository root (directory containing .git).
    
    Returns:
        dict: A GitStatus mapping with keys:
            - branch (str): Current branch name, or "unknown"/"timeout"/"error" on failure.
